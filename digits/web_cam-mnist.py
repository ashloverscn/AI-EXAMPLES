...
def enhance_and_preprocess_roi(roi_bgr):
    """
    Applies image enhancement filters to make the digit prominent,
    ...
    # 2. CLAHE (Contrast Limited Adaptive Histogram Equalization): Enhances contrast of faint digits
...
def main():
    ...
    webcam_index = 0
    print(f"Connecting to webcam (index {webcam_index})...")
    
    cap = cv2.VideoCapture(webcam_index)
    if not cap.isOpened():
        print("[Error] Could not open webcam. Check if another application is using it.")
        return

    window_name = 'Webcam Filtered Digit Recognition'
    cv2.namedWindow(window_name, cv2.WINDOW_AUTOSIZE)

    print("\n[INFO] Webcam connected successfully!")
    print("Controls:")
    print(" - Place your digit inside the green box.")
    ...
    while cap.isOpened():
        ...
        # Optionally flip horizontally for a mirror view
        ...
        # Show the real-time thresholded/filtered binary view in the corner
        ...
        detected_text = "Waiting for digit..."
        ...
        if input_tensor is not None:
            with torch.no_grad():
                ...
                if conf_score > 0.60:
                    detected_text = f"Digit: {pred_digit} ({conf_score*100:.1f}%)"
