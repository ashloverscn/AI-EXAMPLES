...
class ResizableOverlay(QWidget):
    ...
    def run_inference(self):
        try:
            ...
            blob = cv2.dnn.blobFromImage(bgr_frame, 1.0, (300, 300), [104.0, 117.0, 123.0], False, False)
            ...
            # Loop over detections to find the highest confidence face inside the overlay
            for i in range(detections.shape[2]):
                confidence = detections[0, 0, i, 2]
                if confidence > DETECTION_THRESHOLD and confidence > best_confidence:
...
def main():
    ...
    # 3. Initialize OpenCV Caffe Face Detector
